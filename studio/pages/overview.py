    ...
    if docs.empty:
        ...
        with st.container(border=True):
            ...
            if st.button('Запустить проверку проекта',type='primary',width='stretch',disabled=not prepared or bool(errors) or not confirmed):
                ...
                try:
                    st.session_state.result=ctx.analyze(files,ctx.config_dir,progress_callback=update_progress,ai_options={'level':{'Отключён':'off','Умный автоматический':'extended','Помощник':'helper','Расширенный':'extended','Максимальный':'maximum'}.get(str(st.session_state.get('ai_pipeline_level') or 'Помощник'),'helper'),'provider':provider_for_role('extraction',st.session_state,st.secrets) if str(st.session_state.get('ai_pipeline_level') or 'Помощник')!='Отключён' else None,'judge_provider':provider_for_role('judge',st.session_state,st.secrets) if str(st.session_state.get('ai_pipeline_level') or 'Помощник')!='Отключён' else None,'critic_provider':provider_for_role('critic',st.session_state,st.secrets) if str(st.session_state.get('ai_pipeline_level') or 'Помощник')!='Отключён' else None,'semantic_checkpoint':st.session_state.setdefault('semantic_execution_checkpoint',{})})
