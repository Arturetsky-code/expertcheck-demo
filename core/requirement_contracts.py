...
TEXT_SECTION_HINTS=(
 (('генеральн план','планировочн','внутриплощадочн','проезд','подтоплен','благоустрой','огражден','земельн участ','рельеф','водоотводн канав','подпорн стен'),['ПЗУ']),
 (('технологическ','оборудован','производительност','дроблен','конвейер','погрузчик','самосвал','бункер'),['ТХ']),
 (('архитектурн','помещен','фасад','ограждающ конструкц','навес','блочно-модульн'),['АР','ПЗ']),
...
CRITICAL_QUALIFIER_STEMS = (
 ...
 'онлайн','аварийн','перегруз','вахтов','мачт','ливнев',
...
def infer_scope(requirement:dict[str,Any])->str:
    ...
    site_feature_context=any(marker in text for marker in (
        ...
        'внутриплощадочн','благоустройств','дорожн сеть предприятия','подпорн стен',
    ))
    ...
    if code=='CAPACITY' and any(x in text for x in (
        'проектная производительность','проектная мощность','мощность проекта',
        'производительность проекта','мощность предприятия','производительность предприятия',
        'производственная мощность','годовая производительность','годовая мощность',
        'тыс. тонн в год','тыс т/год','млн. тонн в год','млн т/год',
    )):
        return SCOPE_PROJECT
