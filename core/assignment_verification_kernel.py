...
def _stockpile_complex_access_check(requirement: dict[str, Any], page_corpus: list[dict[str, Any]]) -> dict[str, Any] | None:
    """Verify vehicle + personnel connection from the raw-ore stockpile to the process complex.

    This route is intentionally narrow. Generic roads, generic pedestrian paths or a
    generic stockpile mention are insufficient. Categorical proof requires both endpoint
    topology and independent transport/pedestrian connection evidence.
    """
    text=str(requirement.get("requirement_text") or "")
    low_req=_norm(text)
    if str(requirement.get("requirement_type") or "") != "PRESENCE_REQUIREMENT":
        return None
    required_markers=("проезд техник","проход персонал","склад недроблен","технологическ комплекс")
    if not all(marker in low_req for marker in required_markers):
        return None

    pzu_pages=_candidate_pages(page_corpus, ("ПЗУ",))
    th_pages=_candidate_pages(page_corpus, ("ТХ",))
    slots: dict[str, tuple[dict[str, Any], str] | None] = {
        "endpoint_topology": None,
        "pedestrian_text_link": None,
        "pedestrian_drawing_link": None,
        "vehicle_operational_link": None,
        "vehicle_drawing_link": None,
    }

    for page in pzu_pages:
        raw=str(page.get("text") or "")
        low=_norm(raw)
        if slots["endpoint_topology"] is None and (
            "верхн" in low
            and "склад недроблен" in low
            and "технологическ комплекс" in low
            and any(x in low for x in ("связующ", "центральн", "основн назначен"))
        ):
            slots["endpoint_topology"]=(page,_context(raw,("верхн","склад недроблен","технологическ комплекс"),radius=520))

        if slots["pedestrian_text_link"] is None and (
            "пешеходн" in low
            and "верхн" in low
            and "рудн" in low
            ...
            and "лестниц" in low
            and "технологическ комплекс" in low
            and any(x in low for x in ("осуществляется", "выполняется"))
        ):
            slots["pedestrian_text_link"]=(page,_context(raw,("пешеходн","рудн","лестниц","технологическ комплекс"),radius=560))

        if slots["pedestrian_drawing_link"] is None and (
            "металлическая лестница" in low
            and "склад недроблен" in low
            and "4.2 технологический комплекс" in low
            and any(x in low for x in ("схема планировочной организации", "план благоустройства", "разбивочный план"))
        ):
            slots["pedestrian_drawing_link"]=(page,_context(raw,("металлическая лестница","склад недроблен","4.2 технологический комплекс"),radius=520))

    for page in th_pages:
        raw=str(page.get("text") or "")
        low=_norm(raw)
        if slots["vehicle_operational_link"] is None and (
            "отгрузк" in low
            and ("рудн" in low or "склад" in low)
            and "фронтальн" in low
            and "погрузчик" in low
            and "приемн" in low
            and "бункер" in low
        ):
            slots["vehicle_operational_link"]=(page,_context(raw,("отгрузк","фронтальн","погрузчик","приемн","бункер"),radius=560))

        if slots["vehicle_drawing_link"] is None and (
            "схема отгрузки склада" in low
            and "погрузки в бункер дск" in low
            and "колесн" in low
            and "погрузчик" in low
            and "бункер" in low
        ):
            slots["vehicle_drawing_link"]=(page,_context(raw,("схема отгрузки склада","погрузчик","бункер"),radius=560))

    labels={
        "endpoint_topology":"склад недроблёной руды и технологический комплекс связаны общей планировочной схемой",
        "pedestrian_text_link":"адресная пешеходная связь рудного склада через лестницы у технологического комплекса",
        "pedestrian_drawing_link":"графическое подтверждение металлической лестницы у склада и технологического комплекса",
        "vehicle_operational_link":"отгрузка рудного склада фронтальным погрузчиком в приёмный бункер",
        "vehicle_drawing_link":"графическая схема отгрузки склада и погрузки в бункер ДСК",
    }
    ...
    if all_proven:
        for key in ("pedestrian_text_link","vehicle_operational_link"):
            page,snippet=slots[key]  # type: ignore[misc]
            evidence_rows.append({
                "evidence_kind":"QUALIFIED_STOCKPILE_COMPLEX_ACCESS",
                "evidence_state":"verified_candidate",
    ...
    return {
        ...
        "decision_basis":(
            "Проезд техники и проход персонала между рудным складом и технологическим комплексом подтверждены независимыми ПЗУ/ТХ доказательствами."
            if all_proven else
            "Связь склада недроблёной руды с технологическим комплексом подтверждена частично; отсутствуют: "
            + ", ".join(labels[key] for key in missing)
        ),
        "verification_kernel":"STOCKPILE_COMPLEX_ACCESS_EXECUTOR",
        "condition_matrix":[
...
def verify_assignment_requirement(requirement: dict[str, Any], page_corpus: list[dict[str, Any]]) -> dict[str, Any] | None:
    ...
    checkers=(
        ...
        _stockpile_complex_access_check,
