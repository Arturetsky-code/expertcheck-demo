    ...
    with st.expander('Цифровой снимок для повторной проверки', expanded=False):
        ...
        st.download_button(
            ...
            data=project_snapshot_bytes(
                ...
                semantic_checkpoint=st.session_state.get('semantic_execution_checkpoint') or {},
                workspace_state={
                    'project_name': st.session_state.get('project_name'),
                    'analysis_time': st.session_state.get('analysis_time'),
                    'object_registry_confirmed': st.session_state.get('object_registry_confirmed', False),
                    'object_assembly_rows': st.session_state.get('object_assembly_rows') or [],
                    'completeness_user_confirmed': st.session_state.get('completeness_user_confirmed', False),
                    'completeness_decisions': st.session_state.get('completeness_decisions') or {},
                    'checklist_run': st.session_state.get('checklist_run'),
                    'checklist_user_results': st.session_state.get('checklist_user_results') or {},
                    'risk_user_decisions': st.session_state.get('risk_user_decisions') or {},
                    'object_learning_examples': st.session_state.get('object_learning_examples') or [],
                },
            ),
