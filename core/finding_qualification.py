...
def qualify_comparison(row: dict[str, Any]) -> dict[str, Any]:
    ...
    if "расхожд" in status or "конфликт" in status:
        if trusted >= 2 and sections >= 2 and source_text:
            return {"finding_class":"CONFIRMED_ISSUE","user_status":"Выявлено несоответствие","risk_eligible":True,"max_risk_level":"Высокий","reason":"Есть независимые доверенные доказательства из нескольких разделов."}
        return {"finding_class":"REVIEW","user_status":"Требует проверки","risk_eligible":True,"max_risk_level":"Средний","reason":"Есть различие, но недостаточно независимых доверенных доказательств."}
    if any(x in status for x in ("недостат", "нет данных", "не подтвержд", "отсутств")):
        return {"finding_class":"INSUFFICIENT_DATA","user_status":"Недостаточно данных","risk_eligible":False,"max_risk_level":"Недостаточно данных","reason":"Отсутствие найденного подтверждения не доказывает отсутствие сведений в проекте."}
    if "требует" in status:
        return {"finding_class":"REVIEW","user_status":"Требует проверки","risk_eligible":False,"max_risk_level":"Низкий","reason":"Диагностический статус требует проверки специалистом и сам по себе не является замечанием."}
