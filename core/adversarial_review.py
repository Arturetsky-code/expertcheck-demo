...
def adversarial_gate(result:dict[str,Any], evidence:list[dict[str,Any]]|None=None, critic:dict[str,Any]|None=None)->dict[str,Any]:
    ...
    strong=[e for e in evidence if e.get('kind') in {'STRUCTURED_FACT','STRUCTURED_CONFLICT'} and e.get('retrieval_score',0)>=65]
    concerns=list((critic or {}).get('concerns') or [])
    contradiction=any(str(e.get('judge_verdict') or '').upper()=='CONTRADICTS' for e in evidence)
